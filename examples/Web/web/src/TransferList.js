...
const remove = (direction, username, file) => {
                    ...
                    <Table.Body>
                        {files.sort((a, b) => getFileName(a.filename).localeCompare(getFileName(b.filename))).map((f, i) => 
                            <Table.Row key={i}>
                                ...
                                {direction === 'download' && isStateRetryable(f.state) && <Table.Cell className='transferlist-retry'><Button size='mini' style={{ padding: 8, width: 60 }} onClick={() => downloadOne(username, f)}>Retry</Button></Table.Cell>}
                                {isStateCancellable(f.state) && <Table.Cell className='transferlist-cancel'><Button size='mini' style={{ padding: 8, width: 60 }} onClick={() => cancel(direction, username, f)}>Cancel</Button></Table.Cell>}
                                {isStateRemovable(f.state) && <Table.Cell><Button size='mini' style={{ padding: 8, width: 60 }} onClick={() => remove(direction, username, f)}>Remove</Button></Table.Cell>}
                            </Table.Row>
                        )}
                    </Table.Body>
                </Table>
