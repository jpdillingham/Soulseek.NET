...
namespace Soulseek.Tests.Unit.Client
{
    ...
    {
        [Trait("Category", "SenRoomMessageAsync")]
        ...
        public async Task SendPrivateMessageAsync_Throws_InvalidOperationException_When_Not_Logged_In(string roomName, string message)
        {
            using (var s = new SoulseekClient())
            {
        ...
        [InlineData("username", "  ")]
        [InlineData("username", "")]
        public async Task SendPrivateMessageAsync_Throws_(string roomName, string message)
        {
            using (var s = new SoulseekClient())
            {
                s.SetProperty("State", SoulseekClientStates.Connected);
        ...
        public async Task SendPrivateMessageAsync_Does_Not_Throw_When_Write_Does_Not_Throw(string roomName, string message)
        {
        ...
        [Trait("Category", "SendRoomMessageAsync")]
        [Theory(DisplayName = "SendRoomMessageAsync throws PrivateMessageException when write throws"), AutoData]
        public async Task SendPrivateMessageAsync_Throws_PrivateMessageException_When_Write_Throws(string roomName, string message)
        {
            var conn = new Mock<IMessageConnection>();
        ...
        public async Task SendPrivateMessageAsync_Throws_TimeoutException_On_Timeout(string roomName, string message)
        {
            var conn = new Mock<IMessageConnection>();
            conn.Setup(m => m.WriteAsync(It.IsAny<byte[]>(), It.IsAny<CancellationToken>()))
        ...
        public async Task SendPrivateMessageAsync_Throws_OperationCanceledException_On_Cancellation(string roomName, string message)
        {
            var conn = new Mock<IMessageConnection>();
            conn.Setup(m => m.WriteAsync(It.IsAny<byte[]>(), It.IsAny<CancellationToken>()))
